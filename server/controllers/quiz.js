...
const addQuestion = async (req, res) => {
  ...
  const { questionType, question, pointType, answerList } = req.body;
  let quiz;
  try {
    ...
    const questionIndex = quiz.questionList.length;
...
const updateQuiz = async (req, res) => {
  ...
  const { name, description, pointsPerQuestion, isPublic, questionList } =
    req.body;
  const quiz = new Quiz({
    _id: id,
    name,
...
const commentQuiz = async (req, res) => {
  const { id } = req.params;
  const { publisherFirstName, publisherLastName, comment } = req.body;
